...
func _init(obj: Object, d, p):
	...
	var same_head := _have_same_head(all_info)
	if same_head:
		var label := Label.new()
		label.text = same_head
		add_child(label)
...
# check if items have the same head
# then trim it and return it, so it can be a lable instead
func _have_same_head(items: Array) -> String:
	if len(items):
		var all_text_same := true
		for i in range(1, len(items)):
			if items[i].text != items[0].text:
				all_text_same = false
				break
		if all_text_same:
			return ""
		var out := ""
		# only compare words, not characters
		var parts := items.map(func(x): return x.text.split(" "))
		var same := []
		# only compare against first item
		for i in len(parts[0]):
			# check that all children have the same
			var all_same := true
			for j in len(items):
				if i >= len(parts[j]) or parts[0][i] != parts[j][i]:
					all_same = false
					break
			# if so, add it
			if all_same:
				same.append(parts[0][i])
			else:
				break
		# was anything similar?
		if same:
			# remove it from the buttons text
			for i in len(items):
				items[i].text = " ".join(parts[i].slice(len(same)))
			return " ".join(same)
		else:
			return ""
	else:
